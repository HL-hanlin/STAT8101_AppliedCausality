...
class VQVAE2Model(base.GenerativeModel):
    ...
    def __init__(
        ...
        self._encoder_b = Encoder( in_channels, bottom_dim, hidden_channels, n_residual_blocks, residual_channels)
        
        self._encoder_t = vaes.Encoder(
            in_channels=bottom_dim,
            out_channels=top_dim,
        ...
        self._quantizer_t = VectorQuantizer(embedding_dim = embedding_dim, num_embeddings = num_embeddings, commitment_cost = commitment_cost)
        self._quantizer_b = VectorQuantizer(embedding_dim = embedding_dim, num_embeddings = num_embeddings, commitment_cost = commitment_cost)

        self._decoder_t = vaes.Decoder(
            in_channels=embedding_dim,
            out_channels=bottom_dim,
            hidden_channels=hidden_channels,
            n_residual_blocks=n_residual_blocks,
            residual_channels=residual_channels,
            stride=2 )
            
        self._conv = nn.Conv2d( in_channels=bottom_dim, out_channels=embedding_dim, kernel_size=1)

        self._decoder_b = Decoder(  out_channels, 2*embedding_dim, hidden_channels, n_residual_blocks, residual_channels)
    ...
    def forward(self, x):
        ...
        quantized_t, vq_loss_t, _, _, encoding_indices_t, _ = self._quantizer_t(encoded_t)
        quantized_b, vq_loss_b, _, _, encoding_indices_b, _ = self._quantizer_b(encoded_b)

        decoded_t = self._decoder_t(quantized_t)
        x_recon = self._decoder_b(torch.cat((self._conv(decoded_t), quantized_b), dim=1))
        
        return encoded_b, encoded_t, x_recon, 0.5*(vq_loss_b + vq_loss_t) + F.mse_loss(decoded_t, encoded_b), quantized_b, quantized_t, encoding_indices_b, encoding_indices_t
