...
class iVAE_IRMStudent(BaseStudent, CUDAAgent):
    def __init__(
        self,
        env,
        vae_wrapper,
        ...
        super(iVAE_IRMStudent, self).__init__(
            env=env,
        ...
        #self.policy_opt = optim.Adam( list(causal_features_encoder.parameters()) + list(policy_network.parameters()), lr=self.adam_alpha)
        self.policy_opt = optim.Adam(  list(policy_network.parameters()), lr=self.adam_alpha)
        #self.obs_to_latent_opt = optim.Adam( list(phase3_obs_to_latent_encoder.parameters()), lr=self.adam_alpha)
    ...
    """
    def phase3_init(self, causal_features_encoder, policy_network,):
    ...
    def infer_latent_from_observation(self, state_env, phase3_latent_samples = 100000, lambda_1 = 10, lambda_2 = 0.01, lr = 0.005, num_iters = 750 ):
        """
        pa_list = self.vae_wrapper.pa_list
        mask_list =  list(set(np.arange(self.vae_wrapper.latent_dim)).difference(self.vae_wrapper.pa_list))
        
        x_single = torch.FloatTensor(state_env)[None,:].to(self.device)
        x = torch.FloatTensor(np.tile(state_env, (phase3_latent_samples, 1))).to(self.device)
        
        sampled_data = self.vae_wrapper.phase2_sampled_data(phase3_latent_samples)
        phase3_latents_pool = sampled_data[1]
        z = torch.FloatTensor(phase3_latents_pool).to(self.device) 
        
        
        # find initial latent z to start with 
        decoder_params = self.vae_wrapper.model.decoder_params(z)
        log_px_z = self.vae_wrapper.model.decoder_dist.log_pdf(x, *decoder_params)
        ...
        obj_func = log_px_z.reshape(-1,1) + lambda_1 * (torch.norm(Zp, dim = 1)**2).reshape(-1,1) + lambda_2 * (torch.norm(Zc, dim = 1)**2).reshape(-1,1)
        initial_pos = torch.argmax(obj_func).cpu().tolist() 
        
        Zp_ = Zp[initial_pos]
        Zc_ = Zc[initial_pos]
        
        Z_cat = Zp_ + Zc_
        Z_opt = Z_cat.clone().detach().requires_grad_(True).to(self.device)
        ...
        for i in range(num_iters):
            decoder_params = self.vae_wrapper.model.decoder_params(Z_opt)
            log_px_z = self.vae_wrapper.model.decoder_dist.log_pdf(x_single, *decoder_params)
        
            loss =  -( log_px_z) + lambda_1 * torch.norm(Z_opt[pa_list])**2 + lambda_2 * torch.norm(Z_opt[mask_list])**2  #)
            opti.zero_grad()
            loss.backward()
            opti.step()
            
            
        return torch.FloatTensor(Z_opt.detach().cpu()).to(self.device)
        """
    ...
    def select_action(self, state, eval_mode=False):
        ...
        if self.latent_norm == None:
            s, l, a = self.vae_wrapper.phase2_sampled_data(100000)
            self.latent_norm = np.linalg.norm(l, axis = 1).mean()
        
        inferred_latent = self.infer_latent_from_observation(state)
        #inferred_latent[self.masked_states] = 0 # mask out Zc 
        #inferred_latent = state
        #inferred_latent[4:8] = 0
        #inferred_latent = torch.FloatTensor(inferred_latent).to(self.device)
    ...
    def train(self, num_updates):
        ...
        for update_index in (range(num_updates)):
            ...
            if update_index % 1000 == 0:
                #print('\repoch {}/{}, policy loss {}, obs->latent loss {} \t'.format(update_index, num_updates, policy_loss.detach(), obs_to_latent_loss.detach() ), end="")
